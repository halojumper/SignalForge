...
export default function Results() {
  return (
    <>
      ...
      <section style={{padding:'80px 0',background:'var(--cream)'}}>
        <div className="container">
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:24}}>
            {testimonials.map(t=>(
              <div key={t.name} style={{background:'var(--white)',borderRadius:18,padding:'32px 28px',border:'1px solid rgba(0,0,0,0.06)'}}>
                <div style={{color:'var(--amber)',fontSize:'1rem',marginBottom:12,letterSpacing:2}}>★★★★★</div>
                <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14}}>
                  {t.chips.map(c=><span key={c} style={{background:'rgba(232,85,42,0.08)',color:'var(--coral)',border:'1px solid rgba(232,85,42,0.18)',borderRadius:50,padding:'4px 12px',fontSize:'0.75rem',fontWeight:700,fontFamily:'Syne,sans-serif'}}>{c}</span>)}
                </div>
                <p style={{fontSize:'0.95rem',lineHeight:1.75,color:'var(--text)',marginBottom:24,fontStyle:'italic'}}>"{t.quote}"</p>
                <div style={{display:'flex',alignItems:'center',gap:12}}>
                  <div style={{width:44,height:44,borderRadius:'50%',background:`linear-gradient(135deg,${t.grad})`,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'0.9rem',color:'var(--white)',flexShrink:0}}>{t.initials}</div>
                  <div>
                    <div style={{fontFamily:'Syne,sans-serif',fontSize:'0.9rem',fontWeight:700,color:'var(--text)'}}>{t.name}</div>
                    <div style={{fontSize:'0.78rem',color:'var(--warm-gray)'}}>{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:24}}>
            {cases.map(c=>(
              <div key={c.title} style={{background:'var(--white)',borderRadius:18,overflow:'hidden',border:'1px solid rgba(0,0,0,0.07)'}}>
                <div style={{padding:'28px 24px 20px',background:'linear-gradient(135deg,var(--sand),var(--sand-mid))'}}>
                ...
                <div style={{padding:'20px 24px 28px'}}>
                  {c.metrics.map(m=>(
                    <div key={m.label} style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
                      <div style={{fontFamily:'Syne,sans-serif',fontSize:'1.5rem',fontWeight:800,color:'var(--coral)',minWidth:64}}>{m.val}</div>
      ...
      <div style={{background:'var(--amber)',padding:'72px 0'}}>
        <div className="container">
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
            {[{n:'200',s:'+',l:'Campaigns Delivered'},{n:'3.4',s:'×',l:'Avg. ROI Multiplier'},{n:'98',s:'%',l:'Client Retention Rate'},{n:'48',s:'h',l:'Average Time to Launch'}].map((s,i)=>(
              <div key={s.l} style={{textAlign:'center',padding:'32px 20px',borderRight:i<3?'1px solid rgba(255,255,255,0.3)':undefined}}>
                <div style={{fontFamily:'Syne,sans-serif',fontSize:'3rem',fontWeight:800,color:'var(--white)',lineHeight:1,marginBottom:8}}>{s.n}<span style={{color:'rgba(255,255,255,0.7)'}}>{s.s}</span></div>
                <div style={{fontSize:'0.9rem',color:'rgba(255,255,255,0.85)'}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
